...
async def get_fleet_overview(
    ...
    battery_query = select(func.avg(Device.battery_level)).where(
        Device.tenant_id == tenant_id, Device.battery_level.isnot(None)
    )
    ...
    low_battery_query = select(func.count(Device.id)).where(
        Device.tenant_id == tenant_id, Device.battery_level < 20
    )
...
async def get_alert_trends(
    ...
    total_query = select(func.count(Alarm.id)).where(
        Alarm.tenant_id == tenant_id, Alarm.fired_at >= start_date
    )
    total_alarms = (await session.execute(total_query)).scalar()

    # Severity distribution
    severity_query = (
        select(Alarm.severity, func.count(Alarm.id).label("count"))
        .where(Alarm.tenant_id == tenant_id, Alarm.fired_at >= start_date)
    ...
    # Status distribution
    status_query = (
        select(Alarm.status, func.count(Alarm.id).label("count"))
        .where(Alarm.tenant_id == tenant_id, Alarm.fired_at >= start_date)
    ...
    top_devices_query = (
        select(Alarm.device_id, Device.name, func.count(Alarm.id).label("count"))
        .join(Device, Device.id == Alarm.device_id, isouter=True)
        .where(Alarm.tenant_id == tenant_id, Alarm.fired_at >= start_date)
        .group_by(Alarm.device_id, Device.name)
    ...
    daily_result = await session.execute(
        daily_query, {"tenant_id": str(tenant_id), "start_date": start_date}
    )
...
async def get_device_uptime(
    ...
    def _is_effectively_online(d) -> bool:
        ...
        threshold = (
            ...
            if d.device_type_id
            else DEFAULT_OFFLINE_THRESHOLD_SECONDS
        )
    ...
    active_devices = len(
        [
            ...
            for d in devices
            if d.last_seen
            and (d.last_seen if d.last_seen.tzinfo else d.last_seen.replace(tzinfo=timezone.utc))
            >= cutoff_date
        ]
    )

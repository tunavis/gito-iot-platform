...
class DeviceGroupService:
    ...
    def create_group(self, data: DeviceGroupCreate) -> DeviceGroupResponse:
        ...
        existing = self.session.execute(
            select(DeviceGroup).where(
                and_(DeviceGroup.tenant_id == self.tenant_id, DeviceGroup.name == data.name)
            )
        ).scalar_one_or_none()
    ...
    def get_group(self, group_id: UUID) -> Optional[DeviceGroupResponse]:
        ...
        group = self.session.execute(
            select(DeviceGroup).where(
                and_(DeviceGroup.id == group_id, DeviceGroup.tenant_id == self.tenant_id)
            )
        ).scalar_one_or_none()
    ...
    def get_group_with_members(self, group_id: UUID) -> Optional[DeviceGroupDetailResponse]:
        ...
        members = (
            self.session.execute(
                ...
                .where(and_(GroupDevice.group_id == group_id, Device.tenant_id == self.tenant_id))
            )
            .scalars()
            .all()
        )
    ...
    def list_groups(self, skip: int = 0, limit: int = 100) -> Tuple[List[DeviceGroupResponse], int]:
        ...
        total = self.session.execute(
            select(func.count(DeviceGroup.id)).where(DeviceGroup.tenant_id == self.tenant_id)
        ).scalar()
        ...
        groups = (
            self.session.execute(
                select(DeviceGroup)
                .where(DeviceGroup.tenant_id == self.tenant_id)
        ...
        for group in groups:
            ...
            member_count = (
                self.session.execute(
                    select(func.count(GroupDevice.id)).where(GroupDevice.group_id == group.id)
                ).scalar()
                or 0
            )
    ...
    def update_group(
        self, group_id: UUID, data: DeviceGroupUpdate
        ...
        for device_id in device_ids:
            try:
                ...
                device = self.session.execute(
                    select(Device).where(
                        and_(Device.id == device_id, Device.tenant_id == self.tenant_id)
                    )
                ).scalar_one_or_none()
                ...
                existing = self.session.execute(
                    select(GroupDevice).where(
                        and_(GroupDevice.group_id == group_id, GroupDevice.device_id == device_id)
                    )
                ).scalar_one_or_none()
                ...
                # Add to group
                membership = GroupDevice(group_id=group_id, device_id=device_id)
                self.session.add(membership)
                added += 1

            except Exception as e:
                failed += 1
                errors.append({"device_id": str(device_id), "reason": str(e)})

        self.session.commit()

        return BulkDevicesResponse(
            added=added, failed=failed, skipped=skipped, errors=errors if errors else None
        )

    def remove_devices_from_group(
        self, group_id: UUID, device_ids: List[UUID]
    ) -> BulkDevicesResponse:
    ...
    def get_group_members(
        ...
        total = (
            self.session.execute(
                select(func.count(GroupDevice.id)).where(GroupDevice.group_id == group_id)
            ).scalar()
            or 0
        )

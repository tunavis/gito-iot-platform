    ...
    row = (
        (
            await session.execute(
                ...
                {
                    "tid": str(tenant_id),
                    "sid": str(subscription_id) if subscription_id else None,
                    "num": number,
                    "status": status,
                    "cur": currency,
                    "sub": subtotal_cents,
...
async def record_payment(
    ...
    *,
    ...
    await session.execute(
        ...
        {
            "tid": str(tenant_id),
            "inv": str(invoice_id) if invoice_id else None,
            "prov": provider,
            "ref": provider_ref,
            "amt": amount_cents,
            "cur": currency,
...
async def process_webhook(
    session, redis, provider_name: str, headers: dict, raw_body: bytes
) -> dict:
...
async def _record_and_activate(session, redis, provider_name: str, wh: ProviderWebhook) -> dict:
    ...
    inserted = (
        await session.execute(
            ...
            {
                "prov": provider_name,
                "eid": wh.event_id or "unknown",
                "etype": wh.kind,
                "payload": json.dumps(wh.raw),
            },
        )
    ).scalar_one_or_none()
    if inserted is None:
...
async def _handle_payment(session, redis, wh: ProviderWebhook) -> str:
    ...
    if wh.success:
        ...
        if checkout:
            ...
            await sub_service.assign_plan(
                ...
                actor="system:card",
                provider=provider,
                provider_ref=wh.token,
            )
            await _invoice_and_record(
                session,
                checkout["tenant_id"],
                checkout["plan_code"],
                checkout["interval"],
        ...
        if sub:
            ...
            await _invoice_and_record(
                session, sub["tenant_id"], sub["plan_code"], sub["billing_interval"], wh, "paid"
            )
    ...
    if sub:
        await session.execute(
            text(
                "UPDATE subscriptions SET status='past_due', grace_until = :g, updated_at=now() "
                "WHERE id = :id AND status='active'"
            ),
            {"g": datetime.utcnow() + timedelta(days=sub_service.GRACE_DAYS), "id": str(sub["id"])},
        )
        await sub_service._record_event(
            session,
            sub["tenant_id"],
            sub["id"],
            "active",
...
async def _invoice_and_record(
    session, tenant_id, plan_code, interval, wh, invoice_status, failed=False
):
    provider = _card_provider()
    sub = await _sub_by_tenant(session, tenant_id)
    subtotal = await _plan_price_cents(session, plan_code, interval, "ZAR") or (
        wh.amount_cents or 0
    )
    ...
    await record_payment(
        ...
        invoice_id=inv["id"],
        provider=provider,
        provider_ref=wh.event_id,
        amount_cents=(wh.amount_cents or inv["total_cents"]),
        currency="ZAR",
...
async def _sub_by_token(session, token: str | None):
    ...
    return (
        (
            await session.execute(
                text(
                    "SELECT s.id, s.tenant_id, s.billing_interval, p.code AS plan_code "
...
async def charge_due_card_subscriptions(session, redis) -> dict:
    ...
    charged = failed = 0
    for sub in due:
        subtotal = await _plan_price_cents(
            session, sub["plan_code"], sub["billing_interval"], "ZAR"
        )
        if not subtotal:
            continue
        ...
        if result.success:
            ...
            await session.execute(
                text(
                    "UPDATE subscriptions SET status='past_due', grace_until=:g, updated_at=now() WHERE id=:id"
                ),
                {
                    "g": datetime.utcnow() + timedelta(days=sub_service.GRACE_DAYS),
                    "id": str(sub["id"]),
                },
            )
            await sub_service._record_event(

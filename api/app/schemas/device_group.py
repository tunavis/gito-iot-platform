...
class DeviceGroupCreate(BaseModel):
    ...
    group_type: Optional[str] = Field(
        None, max_length=50, description="Group type (logical, physical, functional)"
    )
    membership_rule: Optional[dict] = Field(
        default_factory=dict, description="Dynamic membership rules (tags, status filters, etc.)"
    )
...
class DeviceGroupDetailResponse(DeviceGroupResponse):
    ...
    members: List[DeviceGroupMemberResponse] = Field(
        default_factory=list, description="List of devices in group"
    )

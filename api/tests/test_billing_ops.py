...
class TestProcessWebhook:
    ...
    async def test_invalid_signature_rejected(self, monkeypatch):
        monkeypatch.setattr(billing_ops, "get_provider", lambda n: _FakeProvider(valid=False))
    ...
    async def test_duplicate_is_skipped(self, monkeypatch):
        ...
        monkeypatch.setattr(
            billing_ops, "get_provider", lambda n: _FakeProvider(valid=True, webhook=_wh())
        )
...
class TestHandlePayment:
    ...
    async def test_success_with_checkout_activates_with_token(self, monkeypatch):
        ...
        async def _assign(
            session, redis, tid, *, plan_code, billing_interval, actor, provider, provider_ref
        ):
...
class TestConfirmCheckout:
    ...
    async def test_not_paid_does_not_activate(self, monkeypatch):
        monkeypatch.setattr(
            billing_ops, "get_provider", lambda n: _VerifyProvider(_wh(success=False))
        )
        session = _FakeSession()
        result = await billing_ops.confirm_checkout(session, None, "paystack", "ref1")
    ...
    async def test_success_activates_via_shared_path(self, monkeypatch):
        monkeypatch.setattr(
            billing_ops, "get_provider", lambda n: _VerifyProvider(_wh(success=True))
        )

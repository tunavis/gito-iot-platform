-- ============================================================================
-- Gito IoT Platform - Initial (Genesis) Schema
--
-- FROZEN CONTENT — this file is executed VERBATIM by Alembic migration
-- `api/alembic/versions/001_initial_schema.py` (op.execute(this file's text))
-- as the very first step of every fresh-install migration chain.
--
-- This does NOT represent the platform's current schema. Everything added or
-- changed since the first release lives in migrations 002-022
-- (api/alembic/versions/) and is layered on top of what's here at
-- `alembic upgrade head` time — those later migrations are NOT written
-- defensively (no `IF NOT EXISTS` guards), so pre-creating their tables/columns
-- here would make a fresh install fail partway through the chain.
--
-- DO NOT edit this file to "match current reality." If the schema needs to
-- change, add a new Alembic migration instead. Only touch this file for a
-- deliberate, carefully-verified migration-history squash (rewriting this
-- file AND neutering the migrations it would duplicate in the same change),
-- never as a routine schema update.
--
-- IDEMPOTENT: Safe to run on fresh or existing databases (CREATE ... IF NOT
-- EXISTS throughout) — this property must be preserved by any future edit.
-- ============================================================================

...
- [x] 1.1 Migration: `device_types.driver` (JSONB, nullable). Nullable **is** the
...
- [x] 1.2 A schema for the declaration (transport, commands, telemetry,
...
- [x] 1.3 Driver resolution: device → device type → driver, with a null-driver
      path that returns today's behaviour verbatim.
- [x] 1.4 `_detect_protocol()` consults the driver's `transport.protocol`, then
...
- [x] 1.5 Transport parameters come from the driver (LoRaWAN fPort and confirmed
      flag, MQTT topic, HTTP shape) rather than the hardcoded fPort 201.
- [x] 1.6 `transport.mode` is an **explicit** discriminator
...
- [x] 1.7 Assert that no code outside a driver assumes "a device sends payloads".
...
- [x] 2.1 An encoder that turns command name + parameters into bytes from a
      declarative field spec: offset, length, type, endianness, scaling.
- [x] 2.2 `dispatch()` encodes **before** choosing a transport, so encoding is
      protocol-independent by construction rather than per-transport by accident.
- [x] 2.3 A `passthrough_json` form reproducing today's exact payload, used when
      no driver is present. Byte-identical — assert it.
- [x] 2.4 B METERS IWM and RFM command definitions from
...
- [x] 2.5 Encode tests against the manuals' worked examples. RFM has seven that
...
- [x] 3.1 `response_window_seconds` per driver replaces
      `DEVICE_RESPONSE_TTL_SECONDS` for typed devices; the constant remains only
      as the no-driver default.
- [x] 3.2 `expire_timed_out_commands` honours the per-driver window. An IWM
      command must survive up to 12 hours, not 60 seconds.
- [x] 3.3 An `unacknowledgeable_commands` list, and a terminal state for
...
- [x] 3.4 Declare the receive-window constraint (Class A, ~2-minute post-join
...
- [x] 3b.1 Add a `get_command_status` read tool. `send_device_command` returns an
...
- [x] 3b.2 It must report the honest state, including "delivered, and this device
      cannot confirm" — the terminal state added in 3.3. A tool that only ever
      says `sent` teaches an agent to claim success.
- [x] 3b.3 Confirm `get_device`'s `metrics` are unaffected by phase 2. They come
...
- [x] 4.1 A device type with no driver behaves byte-identically to today, on
      every transport. This is the test that lets phase 1 ship safely.
- [x] 4.2 Encoding is asserted against expected bytes — no network server needed,
      so ChirpStack being unreachable blocks nothing.
- [x] 4.3 An unknown protocol raises rather than publishing to MQTT.
- [x] 4.4 A per-driver window governs expiry; a 12-hour command is still
      outstanding after an hour.
- [x] 4.5 An unacknowledgeable command reaches its terminal state on delivery and
      is never swept to `timed_out`.
- [x] 4.6 **The criterion test**: add a fictional third vendor with a third
...
- [x] 5.1 Absorb `DeviceType.decoder` into `driver.telemetry`. A **move, not a
      rewrite** — the field spec is unchanged, so this is relocation.
- [x] 5.2 Gate: decoding output is byte-identical before and after for every live
      device type. 68 real meters depend on this path; nothing ships until it is.
- [x] 5.3 Correlate responses on **(device, opcode)** with at most one command in
      flight per pair, since no third-party device echoes our `command_id`.
- [x] 5.4 Support both observed confirmation styles: IWM echoes the opcode with
      `C/R/A=0x01`; RFM echoes the full frame and has a real NACK (`0x02 <Index>`).
- [x] 5.5 A NACK moves the command to failed with the device's reason, rather than
      leaving it to time out.

## 6. Script codecs and the sandbox (phase 3)

- [x] 6.1 **Spike first.** Run a hostile codec — infinite loop, memory bomb,
...
- [x] 6.2 Choose a runtime from that evidence. **If none contains hostile input,
...
- [x] 8.2 `CLAUDE.md`: drivers are file-defined and version-controlled, not
...
- [~] 9.1 IWM **decoder** — partly unblocked 2026-08-03 without the supplement.
      Alarm date packing **solved** from nine real captures (day / month /
      year-2000 / 0x00; 54 fields, zero invalid) and shipped as the `0x28`
      answer layout. Uplink port **confirmed** as 1 from 5,449 of our own
      uplinks. Still open: `0x17`'s stated length contradicting its own byte
      table, and the undocumented password mechanism — worth still asking
      `ticket@bmeters.com`. The **encoder was never blocked**.

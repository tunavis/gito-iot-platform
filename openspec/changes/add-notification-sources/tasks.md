...
- [ ] 1.1 Migration `033`: on `notification_queue` add `source_kind TEXT NOT NULL
...
- [ ] 1.2 Add the partial unique index, and leave
      ...
      ```
- [ ] 1.3 Downgrade **refuses** if any row has `alert_event_id IS NULL`, rather
      ...
      incident.
- [ ] 1.4 Model updated to match, in the same commit as the migration.

## 2. Resolving who hears about a platform fault

- [ ] 2.1 `resolve_platform_notification_tenant(session)` — selects tenants with
...
- [ ] 2.2 Zero matches → log and return None. One → return it. More than one →
...
- [ ] 2.3 Unit tests for all three cases. The deployment has exactly one tenant
      and it is management, so zero and many are reachable only in tests.

## 3. The dispatcher's second entry point

- [ ] 3.1 Fix the no-template fallback **first**, on its own. It is the live path
...
- [ ] 3.2 `process_platform_event(source_kind, payload, dedupe_key)` alongside
      ...
      path.
- [ ] 3.3 Throttle non-alarm sources per `(channel, source_kind)`.
      ...
      ceiling.
- [ ] 3.4 Template selection: prefer an enabled template whose `alert_type`
...
- [ ] 3.5 Queue processing handles a row with no alert event without attempting
      an alert lookup.
- [ ] 3.6 **`api/tests/test_notification_dispatcher.py` passes unedited.** If it
...
- [ ] 4.1 Full API suite green (451 as of `0302118`; this change adds to that).
- [ ] 4.2 Fire a real alarm end to end and confirm it still delivers to the one

## Purpose
Gives the agent-originated command gate a human at the other end: a tenant-wide
queue where an authorised person sees what an agent asked for and why, and
approves or refuses it. Backed by `web/src/app/dashboard/approvals/`, the
approve/reject endpoints in `api/app/routers/commands.py`, and the shared role
dependency in `api/app/dependencies.py` (`COMMAND_ROLES`).

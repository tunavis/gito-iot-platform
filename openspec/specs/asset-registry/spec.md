## Purpose
Models the physical things a tenant operates — a pump station, a pump, a motor —
as a tree of assets that devices attach to, so counts and alarms can be read at
the level a person thinks about ("is the station healthy?") rather than only per
device. Backed by `api/app/routers/assets.py`, `api/app/models/asset.py`, and
`devices.asset_id`. Separate from the Org→Site→DeviceGroup hierarchy, which stays
unchanged.

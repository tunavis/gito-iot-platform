...
introduced — the structure is a strict shallow tree, not a general DAG. A user may
still reposition nodes for the session; see the node-drag requirement in
`alarms-and-alerting`, which holds for every canvas and does not persist anything.
